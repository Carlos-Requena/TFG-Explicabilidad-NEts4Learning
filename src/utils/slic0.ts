...
// Arreglos numéricos genéricos (typed array o array normal)
type NumberSeq = Float64Array | number[];
...
export class SLICO extends BaseSegmentation {
  ...
  constructor(imageData: ImageData, options?: SLICOOptions) {
    super(imageData);
    ...
    // Propiedad añadida para exponer el array crudo al pipeline
    this.rawLabels = null;
  ...
  doRGBtoLABConversion(imageData: ImageData): void {
  ...
  getLABXYSeedsForGivenStepSize(step: number, perturb: boolean): void {
    let n = 0;
  ...
  getLABXYSeedsForGivenK(K: number, perturb: boolean): void {
    ...
    let r = 0;
    const kSeedsL: number[] = [];
    const kSeedsA: number[] = [];
    const kSeedsB: number[] = [];
    const kSeedsX: number[] = [];
    const kSeedsY: number[] = [];
    for (let y = 0; y < this.height; ++y) {
      ...
      for (let x = 0; x < this.width; ++x) {
        ...
        kSeedsL.push(this.lvec[i]);
        kSeedsA.push(this.avec[i]);
        kSeedsB.push(this.bvec[i]);
        kSeedsX.push(X);
        kSeedsY.push(Y);
      }
      ++r;
    }
    this.kSeedsL = kSeedsL;
    this.kSeedsA = kSeedsA;
    this.kSeedsB = kSeedsB;
    this.kSeedsX = kSeedsX;
    this.kSeedsY = kSeedsY;
    if (perturb) this.perturbSeeds();
  }
  ...
  performSuperpixelSegmentationVariableSandM(
    ...
    while (numIter < maxIterations) {
      ...
      for (j = 0; j < size; ++j) {
        ...
        if (temp < 0) throw new Error('Assertion error');
  ...
  enforceLabelConnectivity(
    ...
    let c: number, n: number, x: number, y: number, nindex: number;
    let label = 0,
      oindex = 0,
      adjlabel = 0;
  ...
  performSLICOForGivenStepSize(): Int32Array {
    const size = this.width * this.height,
  ...
  performSLICOForGivenK(): Int32Array {
    const size = this.width * this.height,
      kLabels = fillArray(new Int32Array(size), -1);
    this.doRGBtoLABConversion(this.imageData);
    if (this.perturb) this.detectLabEdges();
    ...
    if (this.enforceConnectivity) {
      const nlabels = fillArray(new Int32Array(size), -1);
      this.enforceLabelConnectivity(kLabels, nlabels, this.K);
      for (let i = 0; i < size; ++i) kLabels[i] = nlabels[i];
    }
    return kLabels;
  }
...
function fillArray<T extends { length: number; [n: number]: number }>(
  array: T,
  value: number,
): T {
...
function remapLabels(labels: Int32Array): number {
...
function encodeLabels(labels: Int32Array, data: Uint8ClampedArray): void {
  for (let i = 0; i < labels.length; ++i) {
    const label = labels[i];
...
export const computeSLICzeroMap = (
  imageData: ImageData,
  ...
  const mapArray = slicoEngine.rawLabels as Int32Array;
